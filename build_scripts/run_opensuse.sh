...
sudo su - root -c "virt-install --accelerate --hvm --name $NAME --ram=2000 --disk $OS-$OS_VERSION-$OS_ARCH-$TYPE-$IMAGE_VERSION.img,bus=scsi,size=$IMAGE_SIZE --location=http://download.opensuse.org/distribution/$VERSION/repo/oss/ -x \"install=http://download.opensuse.org/distribution/$VERSION/repo/oss/ autoyast=http://$NODE_IP/opensuse12.1_stratus.xml\" --network bridge=br0 --mac=$MAC_ADDRESS  --noreboot"
